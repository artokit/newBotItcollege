...
def replace_sender(txt):
    if txt:
        return txt.replace('Ashirusafrika', 'rufus_money')
    return txt
...
def get_edit_copy_keyboard(keyboard):
    ...
    if keyboard:
        ...
        for i in reply_markup:
            button = i[0]
            if button.url:
                kb.row(InlineKeyboardButton(text=button.text, url='https://t.me/rufus_money'))
